...
<title>Coordenadas da Staff · <?=e($brandSettings['top_name'])?></title>
...
<main class="commands-public">
  <div class="commands-head">
    ...
    <h1>📍 Coordenadas da Staff</h1>
    <p>Pesquise locais por categoria, subcategoria ou nome e copie o CDS.</p>
  ...
  <section class="commands-panel coord-section">
    <h2>📍 COORDENADAS DA STAFF</h2>
    <p style="margin:-12px 0 20px;color:#9da7b8;line-height:1.6">Pesquise um local, filtre por categoria e copie o CDS com um clique.</p>
    <div class="coord-toolbar">
      <input id="coordSearch" type="search" placeholder="🔎 Pesquisar local ou CDS..." oninput="filterCoordinates()">
      <select id="coordCategory" onchange="updateSubcategories();filterCoordinates()">
        <option value="">Todas as categorias</option>
        <?php foreach($publicCategories as $cat): ?>
          <option value="<?=e($cat['name'])?>"><?=e($cat['icon'])?> <?=e($cat['name'])?></option>
        <?php endforeach; ?>
      </select>
      <select id="coordSubcategory" onchange="filterCoordinates()"><option value="">Todas as subcategorias</option></select>
    </div>
    <div class="coord-count" id="coordCount"></div>
    <?php if(!$coordinateRows): ?>
      <div class="coord-empty">Nenhuma coordenada cadastrada pelo Staff Master.</div>
    <?php else: ?>
      <div class="coord-grid-public" id="coordGrid">
        <?php foreach($coordinateRows as $c): ?>
          <article class="coord-card" data-category="<?=e($c['category'])?>" data-subcategory="<?=e($c['subcategory'])?>" data-search="<?=e(mb_strtolower($c['name'].' '.$c['cds'].' '.$c['category'].' '.$c['subcategory']))?>">
            <div class="coord-card-head"><div class="coord-card-icon"><?=e($c['icon'])?></div><div><h3><?=e($c['name'])?></h3><div class="coord-card-cat"><?=e($c['category'])?> · <?=e($c['subcategory'])?></div></div></div>
            <div class="coord-card-cds"><span style="flex:1"><?=e($c['cds'])?></span><button type="button" class="coord-copy" data-cds="<?=e($c['cds'])?>" onclick="copyCoordinate(this)">📋 Copiar</button></div>
          </article>
        <?php endforeach; ?>
      </div>
    <?php endif; ?>
